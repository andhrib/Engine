...
public:
	...
	void addTexture(const std::string& path, const std::string& textureName);
	void addTextures(const std::vector<std::string>& paths, const std::vector<std::string>& textureNames);
...
private:
	...
	void loadTextureThread(char const* path, unsigned int textureID, int* width, int* height, int* nrComponents, unsigned char** data, GLenum& format);
